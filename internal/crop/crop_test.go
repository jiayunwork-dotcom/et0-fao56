...
// TestSingleKcMultiplies checks the ETc = Kc*ET0 identity for one value.
...
// TestStagedKcSelectsStage checks that the growth day picks the ini/mid/end
...
// TestKcNonPositiveRejected checks that Kc<=0 fails for both modes.
...
// TestStressScalesETc checks Ks in (0,1] and that Ks=1 matches unstressed.
...
// TestStageRatioProportional checks that moving Kc from ini to mid raises
// ETc by the Kc ratio for a fixed ET0.
...
// TestGrowthDayErrors checks staged Kc without a growth day and growth days
// outside the season.
...
// TestDocumentReferenceAgreement checks that a supplied ET0 must agree with
// the weather block of the same document.
...
// TestAridDayExampleAerodynamicDominates loads the bundled arid day and
// checks that the windy day is clearly driven by its aerodynamic term.
