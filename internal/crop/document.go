...
// ReferenceAgreementTolerance is the gap accepted between a supplied ET0
// and the one recomputed from the weather block of the same document.
const ReferenceAgreementTolerance = 1e-6

// Plan is the crop block of a document.
type Plan struct {
...
// Document is the single input format of the CLI, the API and the bundled
// examples: one weather reading, an optional ET0 that was computed
// elsewhere, and an optional crop block.
...
// Outcome bundles what a document can produce.
type Outcome struct {
	Site      string             `json:"site,omitempty"`
...
// LoadBytes parses a document and rejects unknown fields so a misspelt key
...
// Validate checks the document shape before any computation.
...
// Scale returns the coefficient row the document asks for.
...
// Reference computes ET0 from the weather block, or takes the ET0 that was
// supplied. When both are present they must agree.
...
// Evaluate runs the whole document: ET0, the optional wind sweep and, when
// a crop block is present, ETc.
...
// EvaluateCrop runs the document and requires the crop block to be there.
