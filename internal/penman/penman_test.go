...
// TestET0MatchesDocumentedFormula rebuilds the FAO-56 daily fraction from
// the reported Delta, gamma, deficit and wind and compares it with ET0.
...
// TestCalmDegeneration checks that u2=0 removes the aerodynamic term and
// collapses the denominator to delta+gamma.
...
// TestDeficitMonotone checks that raising the saturation deficit es-ea
// while everything else stays fixed never lowers ET0.
...
// TestEnergyReconciliation checks the identity ET0*lambda equals the two
// numerator terms rebuilt in energy units.
...
// TestNegativeWindRejected checks that a negative wind speed is an error.
...
// TestWindHeightConversion checks that a reading above 2 m is scaled down
// and that a reading already at 2 m passes through unchanged.
...
// TestClaimTranspiredContradiction checks that claiming transpiration
// while the available energy and wind give a non-positive ET0 is an error.
...
// TestWindSweepSorts checks that the sweep returns samples in ascending
// wind speed regardless of input order.
