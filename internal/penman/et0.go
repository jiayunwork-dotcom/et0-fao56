...
// Result is the reference evapotranspiration of one reading together with
// every intermediate quantity the report and the audit need.
type Result struct {
...
// Compute evaluates the FAO-56 Penman-Monteith equation for one reading.
...
// Rebuild recomputes ET0 from the stored intermediate quantities using the
// documented formula. Compute and Rebuild must always agree.
...
// LatentHeatFlux returns lambda*ET0 in energy units.
